# NIKON_RAG


这是一个面向尼康相机规格书（specs）与用户手册（manual）的 **LLM+RAG 精准字段抽取**系统。  
特点：**正则优先、LLM 补齐、证据严格对齐、字段级偏好 specs、可批量测试与离线评测**。

个人学习测试使用，无商用。


## 🔢 测试基准结果
```
Gold 字段数: 75
有预测值字段数: 72
Coverage: 96.0%
Exact Match: 90.667%
Loose Match: 92.0%
```
> 注：Coverage=召回/填充能力；Exact Match=标准化完全一致；Loose Match=包含/近似一致。


Example:
```
Q1: Z8 与 Z9 的像素与连拍差异？

==== Answer ====
结论：有效像素相同，最高连拍相同。
| 字段 | Z8 | Z9 |
| - | - | - |
| 有效像素 | 有效像素数 约4,571万 | 有效像素数 约4,571万 |
| 连拍最高每秒幅数 | 约120幅/秒 | 约120幅/秒 |

引用：
Z8 | Nikon_Z8_specs.pdf | p.2
Z8 | Nikon_Z8_specs.pdf | p.3
Z9 | Nikon_Z9_specs.pdf | p.2
Z9 | Nikon_Z9_specs.pdf | p.3

==== Context(证据句子) ====

- 有效像素数 约4,571万 —— 引用：Z8 | Nikon_Z8_specs.pdf | p.2
- 每秒幅数（近似值） ⾼达约120幅/秒 —— 引用：Z8 | Nikon_Z8_specs.pdf | p.3
- 有效像素数 约4,571万 —— 引用：Z9 | Nikon_Z9_specs.pdf | p.2
- 每秒幅数（近似值） ⾼达约120幅/秒 —— 引用：Z9 | Nikon_Z9_specs.pdf | p.3


```


## 🧩 数据格式（jsonl）

`ask.py` 会递归读取 `*.jsonl`，每行建议包含：

```json
{
  "text": "有效像素数 约4,571万 ……",
  "model": "Z8",
  "doc_type": "specs",
  "page": 2,
  "source": "Nikon_Z8_specs.pdf"
}
````

---
## 环境与依赖

**运行环境**
- Python ≥ 3.9  
- 本地 **Ollama** 
- 默认大模型：`qwen2.5:14b-instruct`（已在代码中设为默认值）


```bash
# 1. 创建并激活 conda 环境（Python 3.9）
conda create -n nikon_rag python=3.9
conda activate nikon_rag

# 2. 安装依赖
pip install -r requirements.txt

# 3. 本地大模型（Ollama + Qwen2.5 14B）
ollama pull qwen2.5:14b-instruct
```

## 🚀 快速开始

### 1) 单问题问答（LLM+RAG）

```bash
python src/ask.py "Z8 与 Z9 的像素与连拍差异？"
```

输出包含：

* `==== Answer ====`：Markdown 表格（字段 × 机型）
* `==== Context(证据句子) ====`：每个值对应的一句证据 + 引用（`MODEL | SOURCE | p.PAGE`）

### 2) 批量问答（写入 `outputs/qa_run.txt`）

```bash
python src/ask.py --batch --out outputs/qa_run.txt
```

---

## 📊 评测（Coverage / EM / Loose）

### A. 用现成批量结果评测（不会再次调用 LLM）

```bash
python src/ask.py --batch --out outputs/qa_run.txt
python src/eval.py --report outputs/qa_run.txt --debug
```

### B. 用“问题列表文件”在线评测（会调用 LLM+RAG）

```bash
# 准备问题文件：每行一个问题
cat > tests_20.txt << 'EOF'
Z8 与 Z9 的视频分辨率和帧频对比？
Z6III 的视频压缩与文件格式各支持哪些？
Z5II 与 Z6III 的存储介质与双卡槽差异？
Z7II 的取景器和显示屏规格是什么？
Z8 的机身尺寸（宽×高×厚）与重量是多少？
Z9 的快门速度范围与闪光同步速度？
Zf 的照片文件格式是否支持 HEIF？
D850 与 Z7II 的有效像素与总像素有何不同？
Z8 的 RAW 视频帧尺寸（像素）和帧率有哪些？
Z9 的 N-RAW 与 ProRes RAW 是否支持？列出具体编码。
Z6III 与 Z5II 的最高连拍每秒幅数对比？
Z8 与 Z9 的存储卡类型与卡槽配置差异？
Z5II 的视频画面尺寸（像素）和帧率（用×写法）有哪些？
Zf 的连拍与有效像素对比 Z6III？
Z8 的视频文件格式（容器）是什么？
Z9 的显示屏（LCD）与取景器（EVF）要点？
Z6III 的快门速度与闪光同步速度？
Z7II 的存储介质与双存储卡插槽配置？
D850 的视频分辨率与帧频列表？
Z8 与 Z9 的像素与连拍规格对比（像素、连拍）？
EOF

python src/eval.py --questions tests_20.txt --debug
```

评测输出：

* `outputs/eval_metrics_YYYYMMDD-HHMMSS.json`
* `outputs/eval_errors_YYYYMMDD-HHMMSS.csv`

---

### 🧠 设计要点

1. **正则优先，LLM 补全**  
   针对规格表中格式稳定的字段（像素、连拍等），优先使用精调正则进行结构化抽取；仅在正则缺失时，调用 LLM 在检索到的证据片段内做“缺口填补”。

2. **答案–证据强绑定**  
   LLM 返回统一结构：`{"value","evidence","cite"}`。框架对输出做约束与校验：  
   - `value` 必须是 `evidence` 的子串；  
   - `cite` 必须来自当前检索到的证据头部。  
   这样保证每一个展示给用户的数值，都能直接追溯到原文出处。

3. **字段级文档偏好（specs 优先）**  
   对“机身尺寸/重量”等数值敏感字段，在检索与抽取阶段对 `doc_type == "specs"` 施加额外权重，仅在 specs 缺失时才回退到 manual，减少说明书文案对参数的干扰。

4. **自动构造「金标」用于评测**  
   评测阶段不依赖人工标注，而是仅基于 specs 文档 + 正则抽取得到一份“金标参数表”；模型预测结果按字段与金标逐项比对，计算 Coverage / Exact Match / Loose Match 等指标，评估整个 LLM+RAG 链路的可靠性。


## 📜 许可

仅供学习与作品集展示。PDF 版权归原出版社/尼康所有，请在遵守相应使用条款与著作权政策的前提下使用本项目。
